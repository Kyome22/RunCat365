    ...
    internal class ContextMenuManager : IDisposable
    {
        ...
        )
        {
            ...
            themeMenu.DropDownOpening += (sender, e) =>
            {
                ...
                {
                    if (item.Tag is Theme t) item.Checked = getManualTheme() == t;
                }
            };
            ...
            fpsMaxLimitMenu.DropDownOpening += (sender, e) =>
            {
                foreach (ToolStripMenuItem item in fpsMaxLimitMenu.DropDownItems)
                {
                    if (item.Tag is FPSMaxLimit f) item.Checked = getFPSMaxLimit() == f;
                }
            };
